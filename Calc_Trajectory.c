...
void calculate_trajectory(const int n, const double slew_time, const double start[3], const double end[3], FILE *ofp){
  ...
  double velocity[3]; // constant velocity in cartesian/alpha coordinate system
  ...
  for (int idx = 0; idx<N; idx++){
    pos[idx] = start[idx];
    delta[idx] = (end[idx] - start[idx])/(n-1);
    velocity[idx] =  (end[idx] - start[idx])/slew_time; //constant rate at which x, y, and alpha changes
  }
